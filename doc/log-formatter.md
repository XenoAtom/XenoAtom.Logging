...
Template-based formatters are generated as `record` types. When a formatter exposes configuration, you can use `with { ... }` to override defaults:
...
When consuming packages, reference the generator as an analyzer (so it runs in your project):
...
When working from source with project references, use the same analyzer pattern:

```xml
<ItemGroup>
  <ProjectReference Include="..\\XenoAtom.Logging\\XenoAtom.Logging.csproj" />
  <ProjectReference Include="..\\XenoAtom.Logging.Generators\\XenoAtom.Logging.Generators.csproj"
                    OutputItemType="Analyzer"
                    ReferenceOutputAssembly="false" />
</ItemGroup>
```
